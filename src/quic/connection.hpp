...
//
// Not in this slice (I1b of the design): Retry and tokens, stateless reset, key update,
// path validation and migration, path MTU discovery, 0-RTT, ECN.
#pragma once
...
public:
    ...
    QuicConnection(App& app, EndpointBase& ep, SSL_CTX* ctx, const PacketHeader& initial, const sockaddr_storage& peer,
                   socklen_t peer_len, const Limits& limits, TimePoint now)
        ...
        our_cids_.push_back(ours);
        registered_.push_back(ours);
        registered_.push_back(odcid_);
        unsigned char client_secret[32], server_secret[32];
        ok_ = initial_secrets(odcid_, client_secret, server_secret) && rx_[0].install(Suite::aes128gcm, client_secret, 32, false) &&
        ...
        random_bytes(ours_tp.stateless_reset_token, 16);
    ...
    void receive(unsigned char* data, std::size_t len, const sockaddr_storage& from, socklen_t fromlen, TimePoint now) {
        ...
        if (fromlen != peer_len_ || std::memcmp(&from, &peer_, fromlen) != 0) return;  // a path change: I1b
        bytes_received_ += len;
        last_receive_ = now;
        ...
        while (pos < len && state_ != State::closing) {
            ...
            if (!rx_[si].mask(pkt + h.pn_offset + 4, mask)) break;
            ...
            std::size_t plain = 0;
            if (!rx_[si].open(pn, pkt, hdr_len, pkt + hdr_len, total - hdr_len, plain)) {
            ...
            if (!h.long_form && ((pkt[0] & 0x04) != 0) != key_phase_) {  // a key update: I1b
                break;
            }
    ...
    void produce(SendBatch& batch, TimePoint now) {
        ...
        for (unsigned d = 0; d < 128; ++d) {
            if (!address_validated_ && bytes_sent_ + max_datagram_ > 3 * bytes_received_) break;
            ...
            if (!buf) {  // the wake-up's batch is full: it goes out now and this connection continues
                ep_.flush();
                buf = batch.reserve(max_datagram_);
                if (!buf) break;
            }
            bool wants[kSpaces];
            ...
            for (unsigned i = 0; i < kSpaces; ++i) {
                if (!wants[i]) continue;
                const std::size_t pad_to = (initial || i == 0) && i == last ? kMinInitialDatagram : 0;
                const std::size_t n = build_packet(static_cast<Space>(i), buf + used, max_datagram_ - used, now, pad_to > used ? pad_to - used : 0);
                if (n == 0) continue;
                if (i == 0) initial = true;
                used += n;
            }
...
private:
    ...
    void handle_frame(Space space, const Frame& f, TimePoint now) {
        switch (f.type) {
            ...
            case frame::max_streams_bidi:
                ...
                peer_max_streams_bidi_ = std::max(peer_max_streams_bidi_, f.value);
                return;
            case frame::max_streams_uni:
                if (f.value > (std::uint64_t{1} << 60)) return fail(err::frame_encoding, f.type, "MAX_STREAMS too large", now);
                peer_max_streams_uni_ = std::max(peer_max_streams_uni_, f.value);
                return;
            ...
            case frame::new_connection_id: {
                ...
                for (const PeerCid& c : peer_cids_)
                    if (c.seq == f.value) return;  // a retransmission
                if (peer_cids_.size() >= 4) return fail(err::connection_id_limit, f.type, "too many connection ids", now);
                PeerCid c;
                c.seq = f.value;
                c.cid = f.cid;
                std::memcpy(c.token, f.token, 16);
                peer_cids_.push_back(c);
                if (f.value2 > retire_prior_to_) {
                    retire_prior_to_ = f.value2;
                    for (std::size_t i = 0; i < peer_cids_.size();) {
                        if (peer_cids_[i].seq < retire_prior_to_) {
                            retire_due_.push_back(peer_cids_[i].seq);
                            peer_cids_.erase(peer_cids_.begin() + static_cast<std::ptrdiff_t>(i));
                        } else {
                            ++i;
                        }
                    }
                    if (peer_cid_seq_ < retire_prior_to_ && !peer_cids_.empty()) {
                        peer_cid_ = peer_cids_.front().cid;
                        peer_cid_seq_ = peer_cids_.front().seq;
                    }
                }
                return;
            }
            case frame::retire_connection_id: {
                ...
                if (f.value == our_cid_seq_of(our_cids_.front()) && our_cids_.size() == 1)
                    return fail(err::protocol_violation, f.type, "retiring the only id", now);
                // I1b issues replacements; here the id simply stays valid (RFC 9000 5.1.2 allows a delay).
                return;
            }
    ...
    QuicStream* incoming_stream(std::uint64_t id, TimePoint now) {
        ...
        if (index >= (bidi ? our_max_streams_bidi_ : our_max_streams_uni_)) {
        ...
        if (index < opened) return nullptr;  // opened before and gone: closed
    ...
    void on_handshake_complete(TimePoint now) {
        ...
        // The client's original destination id is done with: only our ids route here now.
        if (registered_.size() > 1) {
            ep_.unmap(registered_[1], &app_);
            registered_.erase(registered_.begin() + 1);
        }
    ...
    void apply_lost() {
        for (const SentItem& it : rec_.lost) {
            switch (it.kind) {
                ...
                case ItemKind::max_streams_uni: max_streams_due_ = true; break;
                ...
                case ItemKind::retire_cid: retire_due_.push_back(it.id); break;
                default: break;
            }
        }
        rec_.lost.clear();
    ...
    bool wants_to_send(Space space, TimePoint now) const noexcept {
        ...
        if (handshake_done_due_ || max_data_due_ || max_streams_due_ || path_responses_ > 0 || !control_.empty() || !credit_.empty() ||
            !retire_due_.empty())
            return true;
    ...
    std::size_t build_packet(Space space, unsigned char* buf, std::size_t cap, TimePoint now, std::size_t pad_to) {
        ...
        if (space == Space::application) {
            ...
            const Cid& scid = our_cids_.front();
        ...
        if (space == Space::application) {
            ...
            if (max_streams_due_) {
                ...
                if (n1) w += n1;
                const std::size_t n2 = n1 ? put_max_streams(payload + w, room - w, false, our_max_streams_uni_) : 0;
                if (n2) w += n2;
                if (n1 && n2) {
                    max_streams_due_ = false;
                    eliciting = in_flight = true;
        ...
        if (ack_written) rec_.on_ack_sent(space);
        ++packets_out;
    ...
    bool copy_stream_bytes(QuicStream& s, std::uint64_t offset, std::size_t len, unsigned char* dst) noexcept {
        ...
        while (done < len) {
            ...
            if (at < s.head.size()) {
                const std::size_t n = std::min<std::size_t>(want, s.head.size() - static_cast<std::size_t>(at));
                std::memcpy(dst + done, s.head.data() + at, n);
                done += n;
                continue;
            }
            const std::uint64_t body_at = at - s.head.size();
            if (!s.mem.empty()) {
    ...
    std::uint64_t our_cid_seq_of(const Cid&) const noexcept { return 0; }
    ...
    std::vector<Cid> our_cids_;     // ours, sequence 0 first
    std::vector<Cid> registered_;   // what the endpoint's table maps here
    std::uint64_t our_cid_seq_next_ = 1;
    std::vector<PeerCid> peer_cids_;
    std::uint64_t retire_prior_to_ = 0;
    ...
    bool max_data_due_ = false, max_streams_due_ = false;
