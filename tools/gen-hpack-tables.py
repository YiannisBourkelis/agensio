#!/usr/bin/env python3
"""Generates the HPACK and QPACK tables from the text of RFC 7541 and RFC 9204 (docs/rfc/):

  src/http/codec_tables.hpp    what both codecs share: the table entry type, the Huffman
                               codes of the 257 symbols (RFC 7541 appendix B, which RFC
                               9204 4.1.2 adopts) and a nibble-driven decoding automaton
  src/http2/hpack_tables.hpp   HPACK's 61-entry static table (RFC 7541 appendix A)
  src/http3/qpack_tables.hpp   QPACK's 99-entry static table (RFC 9204 appendix A)
  tests/hpack_vectors.hpp      the request and response examples of RFC 7541 appendix C
                               as test vectors (encoded bytes, expected fields, table size)
  tests/h2/hpack_huffman.py    the Huffman codes for the Python test client

usage: tools/gen-hpack-tables.py docs/rfc/rfc7541.txt docs/rfc/rfc9204.txt
...
qpack_text = open(sys.argv[2], encoding="utf-8").read().splitlines()
...
# ---- QPACK static table (RFC 9204 appendix A): indexed from 0; a long value wraps over
# continuation rows with an empty index and name ----
qstatic = []
on = False
for line in qpack_text:
    if re.match(r"^Appendix A\.  Static Table", line):
        on = True
        continue
    if on and re.match(r"^Appendix B\.", line):
        break
    if not on:
        continue
    m = re.match(r"^\s*\|\s*(\d+)\s*\|\s*(\S+)\s*\|\s*(.*?)\s*\|\s*$", line)
    if m:
        idx, name, value = int(m.group(1)), m.group(2), m.group(3)
        assert idx == len(qstatic), (idx, name)
        qstatic.append([name, value])
        continue
    c = re.match(r"^\s*\|\s*\|\s*\|\s*(\S.*?)\s*\|\s*$", line)
    if c and qstatic:
        qstatic[-1][1] += " " + c.group(1)
assert len(qstatic) == 99, len(qstatic)
assert qstatic[0] == [":authority", ""] and qstatic[17] == [":method", "GET"] and qstatic[92][0] == "server", qstatic[92]
assert qstatic[85] == ["content-security-policy", "script-src 'none'; object-src 'none'; base-uri 'none'"], qstatic[85]
...
GENERATED = "// Generated by tools/gen-hpack-tables.py from RFC 7541 and RFC 9204 (docs/rfc/). Do not edit.\n"

with open("src/http/codec_tables.hpp", "w", encoding="utf-8") as out:
    out.write(GENERATED)
    out.write("#pragma once\n\n#include <array>\n#include <cstdint>\n#include <string_view>\n\nnamespace agensio::codec {\n\n")
    out.write("// One entry of a static table (HPACK's and QPACK's).\nstruct StaticEntry {\n    std::string_view name;\n    std::string_view value;\n};\n\n")
    out.write("struct HuffmanCode {\n    std::uint32_t code;\n    std::uint8_t bits;\n};\n\n")
    out.write("// Symbol 256 is EOS (RFC 7541 appendix B; RFC 9204 4.1.2 uses the same code).\ninline constexpr std::array<HuffmanCode, 257> kHuffmanCodes = {{\n")
...
with open("tests/h2/hpack_huffman.py", "w", encoding="utf-8") as out:
    ...
    for name, value in static:
        out.write("    (%r, %r),\n" % (name, value))
    out.write("]\n")
print("static %d, qpack static %d, codes %d, states %d, vectors %d" % (len(static), len(qstatic), len(codes), len(internal), len(vectors)))
