...
#
#   bench/laravel/bench.sh [-d 10s] [-c 64] [-t 4] [-s "agensio nginx"] [-u / -u /json] [-r 1]
...
DUR=10s; CONNS=64; THREADS=4; ROUNDS=1; SERVERS="agensio nginx"; URLS=()
FPM=127.0.0.1:9000; REMOTE_ROOT=/var/www/html/public; FPM_CONTAINER=ddev-agensio-laravel-web
PORT_AGENSIO=8073; PORT_NGINX=8074
while getopts 'd:c:t:s:u:r:' o; do
  case $o in
    d) DUR=$OPTARG ;; c) CONNS=$OPTARG ;; t) THREADS=$OPTARG ;; s) SERVERS=$OPTARG ;;
    u) URLS+=("$OPTARG") ;; r) ROUNDS=$OPTARG ;;
    *) exit 2 ;;
  esac
done
[ ${#URLS[@]} -eq 0 ] && URLS=(/ /json)
...
nc -z 127.0.0.1 9000 2>/dev/null || { echo "php-fpm pool $FPM not reachable: ddev start in bench/laravel" >&2; exit 1; }
...
root = "$BED"
app = "laravel"
php = { socket = "$FPM", remote_root = "$REMOTE_ROOT", read_timeout = 60${AGENSIO_PHP_EXTRA:-} }
TOML
...
pids_of() {
  case $1 in
    ...
    fpm) docker top $FPM_CONTAINER -o pid,args 2>/dev/null | awk '/php-fpm: pool agensio/ {print $1}' | paste -sd, - ;;
  esac
}
...
CHILDREN=$(pids_of fpm | tr ',' '\n' | grep -c . || true)
{
  ...
  echo "- app: Laravel $LARAVELV on PHP $PHPV, php-fpm pool \`agensio\` in $FPM_CONTAINER ($CHILDREN static children, TCP $FPM), same pool for both servers"
