...
#
# usage: bench/ab.sh <base-ref> [-r ROUNDS] [-d DURATION] [-t THREADS] [-w WORKERS] [-u URLSPEC ...]
...
BASE_REF="$1"; shift
ROUNDS=2; DURATION=5s; THREADS=4; WORKERS=1; SPECS=()
while getopts "r:d:t:w:u:" opt; do
  case $opt in
    r) ROUNDS=$OPTARG ;; d) DURATION=$OPTARG ;; t) THREADS=$OPTARG ;; w) WORKERS=$OPTARG ;;
    u) SPECS+=("$OPTARG") ;; *) exit 2 ;;
  esac
done
...
start() {  # binary
  ...
  (cd "$BENCH" && exec "$1" -c "$BENCH/tmp/ab-agensio.toml") >>"$RAW/server.log" 2>&1 &
  PID=$!
...
measure() {  # side round -> appends "side round spec cpu_us rps" lines to $RAW/rows
  ...
  for spec in "${SPECS[@]}"; do
    ...
    [ "$proto" = https ] && url="https://127.0.0.1:8443$path" || url="http://127.0.0.1:8080$path"
    curl -sk -o /dev/null "$url"  # warm-up
    t=$THREADS; [ "$conns" -lt "$t" ] && t=$conns
    raw="$RAW/$side-r$round-$proto-${path//\//_}-c$conns.txt"
    c0=$(cpu_seconds)
...

{
  ...
  for spec in "${SPECS[@]}"; do
    ...
    LC_NUMERIC=C awk -v k="$proto $path $conns" -v label="$proto $path c=$conns" '
      ...
      END { if (bn && wn) printf "| %s | %s | %s | %.3f |\n", label, b, w, (ws/wn)/(bs/bn) }' "$RAW/rows"
  done
} > "$OUT"
