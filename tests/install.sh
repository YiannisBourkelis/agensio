...
check "site-copy: the plugin's drop-in lands in wp-content as t7 with the layout's pattern" "0 t7 agensio 640 yes" "$(ctl site-copy t7.test --from wp-content/plugins/demo/index.php --to wp-content/db.php --yes --reason dropin > $T/out; echo -n "$? "; stat -c '%U %G %a' $T/www/t7.test/wp-content/db.php | tr -d '\n'; echo " $(cmp -s $T/www/t7.test/wp-content/plugins/demo/index.php $T/www/t7.test/wp-content/db.php && echo yes)")"
mkdir -p $T/www/t7.test/wp-content/other; chown agensio:agensio $T/www/t7.test/wp-content/other
check "site-copy: a destination directory owned by another account is refused, nothing written" "1 yes no" "$(ctl site-copy t7.test --from wp-content/db.php --to wp-content/other/db.php --yes --reason dropin > $T/out; echo -n "$? "; grep -q 'another account' $T/out && echo -n yes; echo " $([ -e $T/www/t7.test/wp-content/other/db.php ] && echo yes || echo no)")"
