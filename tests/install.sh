...
check "wp-config.php from the archive is t7's alone (0600) and agensio -t agrees" "t7 agensio 600 yes true" "$(stat -c '%U %G %a' $T/www/t7.test/wp-config.php | tr -d '\n'; echo -n " "; grep -q "\"secured\":\[\"$T/www/t7.test/wp-config.php\"\]" <<< "$out" && echo -n yes; echo " $(ctl validate | python3 -c 'import json,sys; print(str(json.load(sys.stdin)["ok"]).lower())')")"
check "site-copy onto wp-config.php keeps it 0600 and the configuration valid (the report's step 5)" "0 0600 t7 agensio 600 true" "$(ctl site-copy t7.test --from index.php --to wp-config.php --overwrite --yes --reason step5 > $T/out; echo -n "$? "; python3 -c 'import json; print(json.load(open("'$T'/out"))["mode"])' | tr -d '\n'; echo -n " "; stat -c '%U %G %a' $T/www/t7.test/wp-config.php | tr -d '\n'; echo " $(ctl validate | python3 -c 'import json,sys; print(str(json.load(sys.stdin)["ok"]).lower())')")"
check "settings: site-update rewrites the pool through the helper; the pool file carries the limits; done says so" "0 512MB 64M 64M 120 yes yes" "$(ctl site-update t7.test --set memory_limit=512M --set max_body_size=64MB --set max_execution_time=120 --yes --reason limits > $T/out; echo -n "$? "; grep -o 'memory_limit\] = [^ ]*' /etc/php/*/fpm/pool.d/agensio-t7.conf | head -1 | cut -d' ' -f3 | tr -d '\n'; echo -n " $(grep -o 'upload_max_filesize\] = [^ ]*' /etc/php/*/fpm/pool.d/agensio-t7.conf | head -1 | cut -d' ' -f3) $(grep -o 'post_max_size\] = [^ ]*' /etc/php/*/fpm/pool.d/agensio-t7.conf | head -1 | cut -d' ' -f3) $(grep -o 'max_execution_time\] = [^ ]*' /etc/php/*/fpm/pool.d/agensio-t7.conf | head -1 | cut -d' ' -f3) "; grep -q '"done":\["site file .*written, agensio reloaded","php-fpm pool agensio-t7:' $T/out && echo -n yes; echo " $(ctl settings t7.test | python3 -c 'import json,sys; d={x["key"]:x for x in json.load(sys.stdin)["settings"]}; print("yes" if d["memory_limit"]["current"]=="512MB" and d["memory_limit"]["source"]=="site" and d["children"]["source"]=="site" else d["memory_limit"])')")"
