...
check "mcp: a viewer's tool list has only the read tools" "7" "$(tools_as carol)"
check "mcp: an operator sees reload, logs-reopen and cert-renew too" "10" "$(tools_as bob)"
check "mcp: an admin sees everything" "15" "$(tools_as alice)"
check "mcp: an account without a role sees the read tools but cannot call them" "7 yes" "$(tools_as dave) $(su -s /bin/sh dave -c "printf '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\",\"params\":{\"name\":\"server_status\",\"arguments\":{}}}\n' | '$BIN' mcp --socket '$T/run/control.sock'" 2>/dev/null | grep -q '"isError":true' && echo yes)"
stop
