...
check "copy: dry_run reports the resolved paths and mode without writing" "200 yes no" "$(cpost /v1/sites/inst.test/install '{"file":"wp.tgz","confirm":true}' > /dev/null; cpost /v1/sites/inst.test/copy '{"from":"plugins/demo/index.php","to":"wp-content/db.php","dry_run":true,"confirm":true}') $(grep -q "\"to\":\"$ROOT/bench/tmp/sites/inst.test/web/wp-content/db.php\"" bench/tmp/ctl-reply.json && echo yes) $([ -e bench/tmp/sites/inst.test/web/wp-content/db.php ] && echo yes || echo no)"
check "copy: the file lands with the directory's pattern and the same bytes; 201; audited" "0 201 yes yes 644" "$("$BIN" ctl site-copy inst.test --from plugins/demo/index.php --to wp-content/db.php --yes --reason dropin --socket $CS > bench/tmp/ctl.out; echo -n "$? "; echo -n "$(cpost /v1/sites/inst.test/copy '{"from":"plugins/demo/index.php","to":"wp-content/db2.php","confirm":true,"reason":"dropin"}') "; cmp -s bench/tmp/sites/inst.test/web/plugins/demo/index.php bench/tmp/sites/inst.test/web/wp-content/db.php && echo -n yes; echo -n " "; grep -q 'copy (dropin): wrote .*wp-content/db.php from .*plugins/demo/index.php' bench/tmp/audit.log && echo -n yes; echo " $(stat -c %a bench/tmp/sites/inst.test/web/wp-content/db.php 2>/dev/null || stat -f %Lp bench/tmp/sites/inst.test/web/wp-content/db.php)")"
check "copy: an existing destination is refused, replaced with overwrite (old size reported, 200)" "409 yes 200 yes" "$(cpost /v1/sites/inst.test/copy '{"from":"plugins/demo/index.php","to":"wp-content/db.php","confirm":true}') $(grep -q 'send overwrite' bench/tmp/ctl-reply.json && echo yes) $(cpost /v1/sites/inst.test/copy '{"from":"plugins/demo/wp-login.php","to":"wp-content/db.php","overwrite":true,"confirm":true}') $(grep -q '"replaced":{"bytes":' bench/tmp/ctl-reply.json && echo yes)"
