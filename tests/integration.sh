...
print(len(deny), n404, readme[0]["handler"] if readme else "-")')"
  ...
  check "grav: none of the private files under user/ leaked a byte" "0" "$(curl -sS $G/user/accounts/admin.yaml $G/user/config/security.yaml $G/user/config/system.yaml $G/user/pages/01.home/default.md $G/user/env/example.com/config/system.yaml $G/user/data/flex/objects/x.json $G/user/accounts/admin/user.yaml | grep -c 'CANARY\|hashed_password\|Draft\|alias')"
  # Grav's user-folder-exposure guidance (learn.getgrav.org/2/security/user-folder-exposure): avatars alone under
  # user/accounts, public media alone under user/data, user/config and user/env whole, no scripts in the public caches.
  check "grav: user/accounts/ answers avatar images and nothing else; user/data/ public media and nothing else, directories included" "200 image/png 404 404 200 application/pdf 200 image/jpeg 404 404" "$(ctype $G/user/accounts/avatars/admin.png) $(code $G/user/accounts/avatars/admin.svg) $(code $G/user/accounts/admin/user.yaml) $(ctype $G/user/data/uploads/doc.pdf) $(ctype $G/user/data/uploads/photo.jpg) $(code $G/user/data/flex/objects/x.json) $(code $G/user/data/uploads/)"
  check "grav: user/config/, user/env/ and webserver-configs/ are never answered whatever the ending; the root's other markdown files are 404" "404 404 404 404 404 404 404" "$(code $G/user/config/site.css) $(code $G/user/env/example.com/config/system.yaml) $(code $G/user/env/x.css) $(code $G/webserver-configs/nginx.conf) $(code $G/CONTRIBUTING.md) $(code $G/CODE_OF_CONDUCT.md) $(code $G/SECURITY.md)"
  check "grav: images/ and assets/ serve their files, never scripts; a missing derivative reaches the front controller" "200 image/jpeg 404 200 text/css 404 grav front /images/missing.jpg" "$(ctype $G/images/pic.jpg) $(code $G/images/x.sh) $(ctype $G/assets/x.css) $(code $G/assets/x.py) $(curl -sS $G/images/missing.jpg)"
  shield_check "grav user/accounts/" $G/user/accounts/avatars tests/grav/user/accounts/avatars
  shield_check "grav user/data/" $G/user/data/uploads tests/grav/user/data/uploads
  ...
  check "grav: every location reported as deny answers 404, files, directories and their bare names; the presets catalogue lists the directories" "30 30 /logs/,/backup/,/cache/,/bin/,/tests/,/tmp/,/webserver-configs/,/user/config/,/user/env/" "$(curl -sS --unix-socket bench/tmp/control.sock http://control/v1/sites/grav.test | python3 -c '
import json,sys,urllib.request
locs=json.load(sys.stdin)["locations"]
deny=[l["path"] for l in locs if l["handler"]=="deny"]
n404=0
...
if [ "$H3" = 1 ] && curl -V 2>/dev/null | grep -q HTTP3; then
  ...
  if python3 -c 'import aioquic' 2>/dev/null; then
    ...
    check "$p attack suite, retry always" "all rows passed" "$(python3 tests/h3-attacks.py "$BIN" --retry always --rows retry,invalid-token,handshake-flood 2>&1 | tail -1)"
  fi
fi
